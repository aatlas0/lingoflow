    ...
    const getAvatarStyles = () => {
        ...
        if (isScenarioMode) {
            switch (disposition) {
                ...
                default: return 'bg-night-card border-night-edge text-night-soft';
            }
        }

        return isHighContrast ? 'bg-night-lift border-night-edge' : 'bg-white border-desert-dark text-desert-dark';
    };
    ...
    const getBubbleClasses = () => {
        ...
        if (isScenarioMode) {
            ...
            switch (disposition) {
                ...
                default: return base + 'bg-night-bg/80 border-night-line text-night-text';
            }
        }

        return (isHighContrast
            ? 'bg-night-card text-white border-night-line'
            ...
            <div className="flex flex-col">
                {isModel && isScenarioMode && npcName && (
                    <span className={`text-xs font-bold mb-1 ml-1 uppercase tracking-wider
                        ${disposition === 'hostile' ? 'text-red-400' : disposition === 'friendly' ? 'text-emerald-400' : 'text-night-muted'}
                    `}>
                        {npcName}
                    </span>
                )}
...
export const ChatView: React.FC = () => {
        ...
        return (
            <div className="flex flex-col h-full w-full max-w-4xl mx-auto p-6 animate-fade-in">
                <div className={`backdrop-blur-md rounded-3xl shadow-xl p-8 border flex flex-col h-full
                    ${isHighContrast ? 'bg-night-card border-night-line' : 'bg-white/80 border-white'}
                `}>
                    ...
                    <div className="flex-grow overflow-y-auto custom-scrollbar mb-6 pr-2">
                        {corrections.length > 0 ? (
                            <div className="space-y-4">
                                <p className={`text-center mb-4 ${isHighContrast ? 'text-night-muted' : 'text-dark-green/70'}`}>Here are the things we corrected today:</p>
                                {corrections.map((c, i) => (
                                    <div key={i} className={`p-4 rounded-xl border shadow-sm ${isHighContrast ? 'bg-night-card border-night-edge' : 'bg-white border-desert-dark/30'}`}>
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className="text-red-500 font-bold">✕</span>
                                            <span className={`line-through ${isHighContrast ? 'text-night-muted' : 'text-dark-green/60'}`}>{c.original}</span>
                                        </div>
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className="text-green-500 font-bold">✓</span>
                                            <span className={`font-bold ${isHighContrast ? 'text-white' : 'text-dark-green'}`}>{c.corrected}</span>
                                        </div>
                                        <p className={`text-sm italic p-2 rounded-lg ${isHighContrast ? 'text-night-soft bg-night-lift' : 'text-dark-green/80 bg-desert/20'}`}>
                                            💡 {c.explanation}
                                        </p>
                                    </div>
                                ))}
                            ...
                            <div className="text-center py-12">
                                ...
                                <p className={`${isHighContrast ? 'text-night-muted' : 'text-dark-green/70'}`}>You didn't make any mistakes. Amazing job!</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex gap-4 justify-center shrink-0">
                        <button
                            ...
                            className={`px-6 py-3 rounded-xl border-2 font-bold transition-colors
                                ${isHighContrast
                                    ? 'border-night-edge text-night-soft hover:bg-night-card'
    ...
    const bgClass = isScenario
        ? 'bg-night-bg'
        : (isHighContrast ? 'bg-night-bg' : 'bg-transparent'); // Default app bg handles standard mode
        ...
        <div className={`flex flex-col h-full w-full max-w-7xl mx-auto p-4 overflow-hidden relative animate-fade-in ${bgClass}`}>
                        ...
                        <div className={`
                            ...
                            ${currentScenario.disposition === 'hostile' ? 'bg-red-900/50 text-red-200' :
                                currentScenario.disposition === 'friendly' ? 'bg-emerald-900/50 text-emerald-200' :
                                    'bg-night-lift/50 text-night-text'}
                        `}>
                                ...
                                <span className={`text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full border
                                    ${currentScenario.disposition === 'hostile' ? 'border-red-500 text-red-400 bg-red-950' :
                                        currentScenario.disposition === 'friendly' ? 'border-emerald-500 text-emerald-400 bg-emerald-950' :
                                            'border-night-edge text-night-muted bg-night-bg'}
                                `}>
                                    {currentScenario.disposition || 'Neutral'}
                                </span>
                            </div>
                            <p className="text-sm text-night-soft line-clamp-1">{currentScenario.objective}</p>
                            <div className="flex items-center gap-2 mt-1 text-xs text-night-muted">
            ...
            <div className="flex-grow flex gap-6 overflow-hidden min-h-0 relative z-10">
                ...
                <div className={`flex-grow md:w-[70%] flex flex-col backdrop-blur-sm border rounded-3xl shadow-xl overflow-hidden relative min-h-0 transition-colors duration-500
                    ${isScenario
                        ? 'bg-night-bg/50 border-night-line'
                        : (isHighContrast ? 'bg-night-card/40 border-night-line' : 'bg-white/40 border-white')}
                `}>
                    ...
                    <div ref={chatContainerRef} className="flex-grow overflow-y-auto p-4 md:p-6 space-y-6 scroll-smooth min-h-0 custom-scrollbar">
                        {history.length === 0 && !currentScenario && (
                            <div className="h-full flex flex-col items-center justify-center text-center opacity-60">
                                <div className="text-6xl mb-4">👋</div>
                                <p className={`text-xl font-medium ${isHighContrast ? 'text-night-muted' : 'text-dark-green'}`}>{t('chat.start')}</p>
                            </div>
                        )}
                        {history.map((msg) => (
                                ...
                                {msg.role === 'model' && (
                                    <button
                                        ...
                                        className={`self-start ml-14 mt-1 text-xs flex items-center gap-1 hover:text-brand-turquoise transition-colors
                                            ${isScenario ? 'text-ink-soft hover:text-night-soft' : (isHighContrast ? 'text-ink-soft' : 'text-dark-green/60')}
                                        `}
                                    >
                                        🔊 Listen
                                    </button>
                        ...
                        {isLoading && (
                            <div className={`self-start ml-14 p-4 rounded-2xl rounded-tl-none shadow-sm inline-block 
                                ${isScenario ? 'bg-night-card border border-night-line' : (isHighContrast ? 'bg-night-card' : 'bg-white')}
                            `}>
                    ...
                    <div className={`p-4 backdrop-blur-md border-t 
                        ${isScenario
                            ? 'bg-night-bg/80 border-night-line'
                            : (isHighContrast ? 'bg-night-card/60 border-night-line' : 'bg-white/60 border-white/50')}
                    `}>
                        <form onSubmit={handleSubmit} className="flex gap-3 w-full">
                            <button
                                ...
                                className={`p-4 rounded-xl transition-all ${isListening
                                    ...
                                    : (isScenario
                                        ? 'bg-night-card border-2 border-night-edge text-white hover:bg-night-lift'
                                        : (isHighContrast ? 'bg-night-card border-2 border-night-edge text-white hover:bg-night-lift' : 'bg-white border-2 border-desert-dark/50 text-dark-green hover:bg-gray-50'))
                                    }`}
                                title="Voice Input"
                            >
                                🎤
                ...
                <div className={`hidden md:block md:w-[30%] shrink-0 backdrop-blur-sm rounded-3xl border shadow-lg overflow-hidden
                    ${isScenario
                        ? 'bg-night-bg/60 border-night-line'
                        : (isHighContrast ? 'bg-night-card/60 border-night-line' : 'bg-white/60 border-white')}
                `}>
